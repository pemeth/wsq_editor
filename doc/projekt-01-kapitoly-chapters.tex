\chapter{Úvod}
  ...
  odtlačkov prstov k automatizovanému spracovaniu však bolo potrebné vykonať výskum nielen do samotných vzorov tvorených papilárnymi líniami.
...
\chapter{Odtlačok prsta} \label{kap:odtlacok}
  ...
  Pri pohľade na prierez kože na obrázku~{\ref{obr:zamsa_a_papily/papily_prierez}} je možné pozorovať prepojenie zamše s papilárnou pokožkou.
  ...
  Táto podsekcia o zaznamenávaní odtlačkov prstov čerpala z \cite{FingerprintSrcBook}.
  ...
  ukončenie, vidličku, hák, kríženie, bočný kontakt, bod, interval, slučku, most a~priesečnú líniu \cite{Drahansky} - niektoré z nich sú znázornené na
  obrázku~{\ref{obr:typy_markantov}}. Mnohé z vymenovaných typov majú rôzne
  ...
  Je vhodné poznamenať, že niektoré páry markantov majú svoje inverzné náprotivky. Túto inverziu je možné vidieť na pároch obrázkov~{\ref{obr:markant_ukoncenie}}
...
\chapter{Spracovanie a analýza snímok odtlačkov prstov} \label{kap:analyza}
  ...
  \section{Spracovanie snímok}\label{sec:spracovanie_snimok}
  ...
  Tento priemer je získaný pomocou integrálneho obrazu, ktorý je predpočítaný
  ...
  má veľkosť $3$$\times{}$$3$ a definuje kruhový orientovaný sled pixelov (zobrazené na obrázku~{\ref{obr:okolie_ZhangSuen}}).
  ...
  vypočíta sa nový smer $\theta{}_c$ a algoritmus začína novú iteráciu. Algoritmus je znázornený na obrázku~{\ref{obr:sledovanie_linii}}.
  ...
  k rozlíšeniu medzi triedami \cite{Handbook}. Avšak prístup na báze pravidiel (viď sekciu~{\ref{sec:rule-based}}) využíva aj informácie o singularitách.
  ...
  okno veľkosti $3$$\times{}$$3$ (znázornené na obrázku~{\ref{obr:okno_orientacie}}). V tomto okne sa vypočítajú rozdiely medzi jednotlivými pármi pixelov
  obklopujúcich pixel $O(i,j)$ v smere proti behu hodinových ručičiek. Toto okno je kruhové, čiže sa počíta aj rozdiel medzi prvým
  ...
  \subsection{Automatizovaná klasifikácia tried odtlačku prsta} \label{sec:rule-based}
  ...
  jednotlivých singularít prítomných na odtlačkoch (zobrazené na obrázku~{\ref{obr:singularity_v_triedach}}). Tento prístup je možné algoritmizovať a aplikovať
...
\chapter{Wavelet Scalar Quantization} \label{kap:wsq}
  ...
  podpásma. Jedna úroveň transformácie je zobrazená na obrázku~{\ref{obr:DWT_uroven}}, kde bloky H predstavujú filtre s horným priepustom,
  ...
  výstupné signály filtrované vo vyznačenom poradí. Vstupný signál $I$ je rozdelený na podpásma párom filtrov horného
  ...
  \begin{figure}[h]
    ...
    \caption{Diagram zobrazujúci jednu úroveň rozkladu vstupnej snímky do korešpondujúcich podpásem.}
  ...
  \begin{figure}[h]
    \centering
    \includegraphics[width=0.24\linewidth]{obrazky-figures/DWT_aproximacia.png}\hfill
    \includegraphics[width=0.24\linewidth]{obrazky-figures/DWT_vodorovne_detaily.png}\hfill
    \includegraphics[width=0.24\linewidth]{obrazky-figures/DWT_vertikalne_detaily.png}\hfill
    \includegraphics[width=0.24\linewidth]{obrazky-figures/DWT_diagonalne_detaily.png}
    \caption{Rozklad snímky odtlačku prsta na štyri skupiny koeficientov aplikáciou jednej úrovne diskrétnej vlnkovej transformácie.
    Snímky zľava doprava: aproximácia, vodorovné detaily, vertikálne detaily a diagonálne detaily.
    Vstupná snímka z~internej databázy STRaDe. Rozklad získaný pomocou programu MATLAB.}
  ...
  Kvantovanie a jeho reverzná operácia pre dekódovanie komprimovaných snímok sú dané rovnicami~{\ref{rov:kvantovanie}} a~{\ref{rov:kvantovanie_reverz}}.
  \begin{equation}
    p_k(m,n) =
    \begin{cases}
...
\chapter{Návrh a implementácia aplikácie} \label{kap:navrh_appky}
  ...
  spracovávaného odtlačku prsta (znázornené na obrázku~{\ref{obr:app}}). Ukážka reflektuje úkony vykonávané na snímke pomocou ovládacieho panelu. Ovládací panel
  ...
  \subsection{\texttt{Ui\_MainWindow} a \texttt{Ui\_ParamWindow}}
  ...
  línie nepreukazujú a preto je na túto snímku aplikované Gaussovské rozostrenie (priemerovanie), čím sa \uv{spomalí} prechod medzi jednotlivými blokmi.
  ...
  \begin{figure}[h]
    ...
    \begin{subfigure}[b]{0.3\linewidth}
      ...
      \caption{}
    \end{subfigure}
    \caption{Snímky frekvencií, kde (a) zobrazuje pôvodnú frekvenčnú snímku a (b) rozostrenú frekvenčnú snímku.}
...
\chapter{Vyhodnotenie} \label{kap:vyhodnotenie}
  ...
  \subsection{Termálny snímač Bergdata FCAT 100}
  ...
  Obrázok~{\ref{obr:vyhodnotenie_termal}} znázorňuje uvedené kroky spracovania snímok z termálneho snímača.
  \begin{figure*}[h]\centering
    \centering
  ...
  \subsection{Optický snímač SAGEM MorphoSmart MSO300}
  ...
  Obrázok~{\ref{obr:vyhodnotenie_optical}} znázorňuje uvedené kroky spracovania snímok z optického snímača.
  \begin{figure*}[h]\centering
    \centering
  ...
  Obrázok~{\ref{obr:vyhodnotenie_capac}} znázorňuje uvedené kroky spracovania snímok z kapacitného snímača.
  \begin{figure*}[h]\centering
    \centering
  ...
  Implementované algoritmy automatickej analýzy boli otestované na manuálne roztriedenej vzorke odtlačkov prstov. V závislosti od faktorov ako kvalita snímky,
  a pri klasifikácii aj od samotnej triedy odtlačku, sa dosahovali rôzne výsledky. Algoritmy analýzy pracujú na snímkoch vysokej kvality korektne.
  Pri analýze markantov sa detegovali často falošné markanty na okrajoch
  oblasti záujmu a v prípade snímky nižšej kvality aj v rámci oblasti záujmu. Klasifikácia dosahovala najlepšie výsledky pri triede oblúk a najhoršie
  pri pravých sľučkách. Vzhľadom k vyššie uvedenému aplikácia spĺňa všetky požiadavky zadania práce.
  ...
  zrýchlenie algoritmov, pričom by sa zachovala prenositeľnosť.