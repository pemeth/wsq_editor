\chapter{Úvod}
  ...
  Digitalizácia záznamov odtlačkov prstov v USA predstavovala veľkú výzvu pre dátové skládky tamojších kriminalistických orgánov, pretože 
  ...
  po kompresii. Ako riešenie Americké FBI implementovalo novú metódu kompresie snímok založenú na diskrétnej vlnkovej transformácii
  vo formáte WSQ. Súborom WSQ a~spôsobu kompresie v tomto formáte sa venujem v kapitole \ref{kap:wsq}.
...
\chapter{Odtlačok prsta} \label{kap:odtlacok}
  Koža sa skladá z troch častí: epidermis, dermis a hypodermis. Výbežky a údolia štruktúr na končekoch prstov, naprieč dlaňami a chodidlami
  ...
  Prvú klasifikáciu vzorov tvorených papilárnymi líniami na končekoch prstov je možné nájsť v dizertácii českého profesora Jana Evangelista Purkyně
  ...
  AFIS. Tento systém bol vyvinutý FBI pre extrakciu markantov odtlačkov prstov automatizovanými metódami za pomoci počítačov.

  Odtlačky prstov je možné skúmať na rôznych úrovniach globálnosti. Vlastne sa jedná o mieru špecifickosti, podľa akej zaraďujeme vlastnosti
  ...
  póry, šírku papilárnych línií, jazvy a poranenia kože, kožné ochorenia, atp. V súčasnosti sa pri automatizovanej analýze detaily na tejto úrovni
  často nespracúvajú. Je to z dôvodu, že pri nižších rozlíšeniach snímok, typicky pod 1000ppi, tieto detaily nie sú pozorovateľné\cite{Handbook}.
  ...
  \begin{itemize}
    ...
    \item \emph{Poškodená nezotaviteľná oblasť} je oblasť, ktorá je natoľko poškodená, že extrakcia informácií z nej je nerentabilná\cite{Hong}.
  \end{itemize}
  ...
  Pretože nie je možné zaručiť, že frekvencia línií je konštantná naprieč celou oblasťou záujmu snímky, snímka sa rozdelí na bloky $w \times w$, 
  ...
  \begin{itemize}
    \item Vstupná snímka sa rozdelí na $w \times w$ bloky
    \item Vypočíta sa gradient Sobelovým operátorom
    \item Odhadne sa lokálna orientácia bloku pomocou rovníc:
          \begin{equation}
            \mathcal{V}_x(i,j) = \sum_{u=i-\frac{w}{2}}^{i+\frac{w}{2}}\sum_{v=j-\frac{w}{2}}^{j+\frac{w}{2}} 2\partial _{x}(u,v) \partial _y(u,v),
          \end{equation}
          \begin{equation}
            \mathcal{V}_y(i,j) = \sum_{u=i-\frac{w}{2}}^{i+\frac{w}{2}}\sum_{v=j-\frac{w}{2}}^{j+\frac{w}{2}} (2\partial ^{2}_{x}(u,v) \partial ^{2}_{y}(u,v)),
          \end{equation}
          \begin{equation}
...
\chapter{Návrh aplikácie} \label{kap:navrh_appky}
  ...
  detekcia singularít, markantov a triedy odtlačku, ukážka filtrov využívaných pri predstpracovávaní odtlačkov prstov.
