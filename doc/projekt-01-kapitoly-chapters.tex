...
\chapter{Návrh a implementácia aplikácie} \label{kap:navrh_appky}
  ...
  \begin{figure}[h]
    ...
    \label{obr:app}
  \end{figure}
  Menu \texttt{File} poskytuje možnosti pre otvorenie súboru a export spracovaných snímok. Menu \texttt{Transformations} má možnosti pre rotovanie a priblíženie
  snímky. Menu \texttt{Image} uvádza voľby pre zobrazenie jednotlivých krokov spracovania ako sú filtrovanie, extrakcia frekvencií apod. a menu
  \texttt{Analysis} zase voľby pre analýzu snímky, čo zahŕňa zobrazenie singularít, markantov, výpis detegovanej triedy a uloženie extrahovaných markantov do
  JSON súboru.
  ...
  podľa priemernej orientácie v tomto bloku tak, aby papilárne línie boli kolmé na os $x$. Natočenie vykonáva funkcia \texttt{scipy.ndimage.rotate}, pomocou
  ktorej je možné natáčať dvojrozmerné polia. V takto natočenom bloku sú následne spriemerované hodnoty po stĺpcoch
  ...
  Extrakcia singularít je riadená funkciou \texttt{poincare}, ktorá využíva metódu extrakcie Poincaré indexom~{\ref{sec:poincare}}. Funkcia prechádza každý
  pixel snímky orientácií
  ...
  Vyhodnotenie prebiehalo na snímkach rovnakých prstov získaných troma rôznymi sernzormi.
  ...
  aj mimo oblasti záujmu. Generovanie masky oblasti záujmu je kvalitné vďaka veľmi nízkemu rozptylu hodnôt v oblasti pozadia a veľmi vysokému rozptylu hodnôt
  v oblasti záujmu.
  ...
  naznačovať zašpinenie alebo chybu snímača. Tento šum mal podobný dopad na Gaborove filtrovanie ako póry pri optickom snímači, čo znamená, že sa falošne
  vytváral priestor uprostred papilárnych línií. Pri okrajoch oblasti záujmu sa prejavuje podobné správanie ako pri termálnom snímači.
  ...
  jednej z automaticky rozoznávaných tried - oblúk, klenutý oblúk, ľavá / pravá slučka a špirály. Ako bolo uvedené v sekcii~{\ref{sec:singul_a_triedy}},
