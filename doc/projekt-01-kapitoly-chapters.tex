...
\chapter{Wavelet Scalar Quantization} \label{kap:wsq}
  ...
  ako je možné vyčítať z~obrázku~{\ref{obr:WSQ_DWT_dekompozicia}}. Rozdelenie do viacerých, než troch blokov je vo WSQ tiež možné s horným limitom obmedzeným
...
\chapter{Návrh a implementácia aplikácie} \label{kap:navrh_appky}
  ...
  metóda Zhang-Suen pre stenčovanie línií. Z dôvodu rozšírenia týchto metód a ich celkovej kvalite výstupov pri využití na snímkoch odtlačkov prstov
  je vhodné ich implementovať.
  ...
  formátov, medzi ktoré patria aj formáty schopné aplikácia načítať.
  ...
  \begin{figure}[h]
    ...
    \label{obr:app}
  \end{figure}
  Menu \emph{File} poskytuje možnosti pre otvorenie súboru a export spracovaných snímok. Menu \emph{Transformations} má možnosti pre rotovanie a priblíženie
  snímky. Menu \emph{Image} uvádza voľby pre zobrazenie jednotlivých krokov spracovania ako sú filtrovanie, extrakcia frekvencií apod. a menu
  \emph{Analysis} zase voľby pre analýzu snímky, čo zahŕňa zobrazenie singularít, markantov, výpis detegovanej triedy a uloženie extrahovaných markantov do
  JSON súboru.

  Hlavné okno aplikácie zobrazuje súčasne načítanú snímku. V prípade, že užívateľ zvolí jednu z možností pre spracovanie alebo analýzu, táto snímka je
  prepísaná novou, ktorá odzrkadľuje vykonanú operáciu. To znamená, že ak užívateľ zvolí zobrazenie orientačnej snímky, tak sa v snímka v hlavnom okne nahradí
  interpretáciou orientačného poľa súčasnej snímky. Toto platí pre všetky úkony spracovania vstupnej snímky. V prípade analýzy sa extrahujú potrebné informácie
  o markantoch alebo singularitách, v závislosti od požiadavky užívateľa, a zobrazí sa pôvodná snímka s vyznačenými markantmi alebo singularitami. Väčsina
  týchto úprav je zobrazená v sekcii~{\ref{sec:algoritmy}} rovnakým spôsobom ako v aplikácii. V dvoch prípadoch spracovania a analýzy sa zobrazí ďalšie
  okno. Pri zobrazení orientácií je možné zvoliť zobrazenie snímky orientácii, ktoré sa zobrazí v hlavnom okne, alebo zobrazenie vykresleného vektorového poľa
  orientácií - toto sa zobrazí v novom okne za pomoci modulu \texttt{matplotlib.pyplot} a jej funkcie \texttt{quiver}. Druhým prípadom, kde sa zobrazí nové
  okno je pri detekcii triedy, po ktorej sa zobrazí okno so správou zdieľajúcou triedu odtlačku prsta.
  ...
  uvádza vhodná veľkosť kernelu ako 11 pixelov.
  Tento kernel sa konvolúciou aplikuje na súčasný pixel a hodnota sa zapíše do nového dvojrozmerného poľa rovnakej veľkosti ako
  ...
  značky na pozíciách extrahovaných markantov (obrázok~{\ref{obr:markanty}}).
  ...
  Extrakcia singularít je riadená funkciou \texttt{poincare}, ktorá využíva metódu extrakcie Poincaré indexom~{\ref{sec:poincare}}. Pred odovzdaním snímky
  orientácií tejto funkcii sa vyhladí Gaussovským filtrom s odchýľkou $\sigma = 14$, vďaka čomu sa snímka zbaví väčšiny falošných fingularít.
  ...
  obrázku~{\ref{obr:singularity}}, kde vyznačenie singularít prebehlo podobne ako pri markantoch pomocou funkcie \texttt{overlay}.
  ...
  implementácií Gaborovho filtra, ktorý bol pôvodne plne cyklický. V takom prípade pri snímke veľkosti $400 \times 400$ trvala aplikácia filtra približne 70 sekúnd.
